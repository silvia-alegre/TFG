...
Aunque la idea principal del aprendizaje automático es esta, econtramos diferentes tipos de aprendizaje automático, dependiendo del tipo de tarea que se quiera llevar a cabo, de la naturaleza de la medida del rendimiento que se utiliza para evaluar el sistema y del tipo de entrenamiento o experiencia que le proporcionamos a este.
...
!!!!!!!!!!!!!! REESCRIBIR TENIENDO EN CUENTA Q SÍ ES LO MISMO Q PERCEPTRÓN MULTICAPA !!!!!!!!!!!!!!!!!!!!
...
En los transformers, estas capas complementan las salidas de la capa de atención. Mientras que la capa de atención procesa cada palabra de la secuencia relacionandola con las demás palabras, las capas \textit{feed forward} procesan cada palabra de manera independiente. Este procesamiento es crucial para mejorar la representación de las características extraídas de la capa de atención.
...
En este trabajo se utilizarán y compararán tres modelos \textit{open-source} preentrenados: el modelo GPT-4, el modelo Gemma y el modelo Llama-3.

\subsubsection{Modelos GPT}

ALARGAR: Los modelos GPT han sido desarrollados por la compañía \textit{OpenAI}. Concretamente, en este trabajo se utilizará el modelo GPT-4

El modelo GPT-4 \cite{openai2024gpt4technicalreport} es el último de los modelos GPT, desarrollados por \textit{OpenAI}.

%Estos son modelos de lenguaje grandes que utilizan la arquitectura de transformer y que están diseñados para predecir la siguiente palabra en una secuencia.

La innovación de este modelo frente a los anteriores es que es un modelo multimodal, es decir, además de aceptar entradas en formato de texto, también es capaz de analizar imágenes. Puede alcanzar resultados a nivel humano en una gran cantidad de tareas en ámbitos profesionales y académicos. Además, GPT-4 ha sido entrenado con un conjunto de datos mucho más grande que sus predecesores. Este conjunto cuenta con más de 10 billones de palabras, 10 veces más que el modelo anterior, GPT-3, e incluye todo tipo de datos, desde libros y artículos hasta páginas web y publicaciones en redes sociales.

\subsubsection{Gemma}

Gemma \cite{gemmateam2024gemmaopenmodelsbased} es un modelo abierto desarrollado por Google, basado en el modelo privado de esta misma compañía, Gemini \cite{geminiteam2024geminifamilyhighlycapable}. Gemma se presenta en dos tamaños: un modelo con 7 billones de parámetros, para despliegue en GPU y TPU, y otro con 2 millones de parámetros, para despliege en CPU. El modelo de 7B está entrenado con 6T de tokens, mientras que el de 2B, con 3T. Los datos de entrenamiento de ambos son datos en inglés, extraídos de documentos web, matemáticas y código. A diferencia de Gemini y GPT-4, estos modelos no son multimodales ni se han entrenado para obtener los mejores resultados en tareas multilingües.

El modelo que se utilizará en este trabajo es el de 7B.

\subsubsection{Llama}

Llama es un conjunto de modelos \textit{open-source} desarrollados por Meta [...]
...
El primer conjunto de datos incluye un total de 212 frases, todas ellas en español. El segundo conjunto consta de 263 frases, disponibles tanto en español como en inglés.
...
\begin{itemize}
	...
	\item \textbf {Conjunto en inglés}: se ha creado a partir de las frases en inglés del segundo conjunto de datos, por lo que contiene un total de 263 frases.
\end{itemize}
...
\begin{displayquote}
\centerline{Lemmas: [lista de palabras clave]}
\centerline{Phrase: [frase generada]}
\end{displayquote}

Por ejemplo:

\begin{displayquote}
\centerline{Lemmas: policía poner un multa}
\centerline{Phrase: El policía puso una multa.}
\end{displayquote}
...
Para evaluar los modelos entrenados se utilizarán las métricas de BLEU y COMET
...
Tal y como se mencionó en la Sección \ref{modelos}, los modelos utilizados para los experimentos han sido Gemma y Llama-3. Para el entrenamiento de ambos se ha utilizado el método de adaptación de LoRA, explicado en la Sección \ref{lora}. Para el testeo de los modelos se ha utilizado el método de \textit{cross-validation}, realizando un total de diez particiones sobre los datos de entrenamiento. A partir de estas se ha creado el conjunto final de predicciones, sobre el cual se han calculado las métricas de BLEU y COMET.

En la primera tanda de experimentos, se han utilizado los siguientes parámetros de LoRA:

\begin{itemize}
	\item Rango = 16, 32, 64
	\item Alfa = 8, 16, 32, 64 
	\item \textit{Dropout} = 0.1
\end{itemize}
