...
\begin{displayquote}
\centerline{"Q: ¿Cuál es la capital de Francia? A: "}
\end{displayquote}
...
\begin{displayquote}
\centerline{$P(w|\text{Q: ¿Cuál es la capital de Francia? A: })$}
\end{displayquote}
...
\begin{displayquote}
\centerline{$P(w|\text{Q: ¿Cuál es la capital de Francia? A: París})$}
\end{displayquote}
...
El algoritmo de \textit{top-k sampling} es una generalización del \textit{greedy decoding} que consiste en elegir las $K$ palabras con mayor probabilidad de la distribución y aplicar \textit{random sampling} solamente sobre este conjunto. Una vez se eligen las $K$ palabras del conjunto, se deben normalizar sus probabilidades de acuerdo al conjunto creado. Así, las probabilidades con las que se trabaja se calculan como:

\begin{equation}
\hat{p}_i = \frac{p_i * \mathbb{1}\{i \leq K\}}{\sum_{j = 1}^K p_j}
\end{equation}

Donde $\mathbb{1}$ es la función característica \cite{nadeem2020systematiccharacterizationsamplingalgorithms}.

\subsubsection{\textit{Nucleus sampling}}

El algoritmo de \textit{nucleus sampling} \cite{holtzman2020curiouscaseneuraltext}, también conocido como \textit{top-p sampling} sigue con la idea de eliminar de la distribución las palabras menos probables, pero en lugar de quedarse con las $K$ palabras más frecuentes, mantiene un el porcentaje $p$ más importante de la muestra. Así, dada una distribución $P(x |x_{1:i-})$, las \textit{top-p} palabras del vocabulario $V^{(p)} \subset V$ como el conjunto más pequeño que cumpla:

\begin{equation}
\sum_{x \in V^{(p)}} P(x | x_{1:i-1}) \ge p
\end{equation}

Al medir la probabilidad en lugar de un número fijo de palabras, se espera que la medida sea más robusta incluso en contextos muy diferentes, que podrian tener resultados muy distintos si se usara un número fijo de palabras. De nuevo, tras seleccionar el conjunto de palabras que se utilizarán en el \textit{sampling} se deben normalizar sus probabilidades de acuerdo al conjunto.
...
En el algoritmo de \textit{tempered sampling} \cite{nadeem2020systematiccharacterizationsamplingalgorithms}, en lugar de crear un subconjunto de palabras del vocabulario lo que se hace es hacer un \textit{reshape} de este. Para ello, se utiliza un parámetro $\tau$, que cumple $0 < \tau < 1$, utilizando la siguiente fórmula:
