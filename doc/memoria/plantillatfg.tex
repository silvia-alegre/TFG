...
\usepackage{pdfpages}
\usepackage[backend=biber, style=numeric, url=false, doi=false, eprint=true]{biblatex}

\DeclareFieldFormat[article,misc,inproceedings]{title}{"#1"}
\DeclareFieldFormat{eprint:arxiv}{arXiv preprint\addspace arXiv:\,#1}

\addbibresource{plantillatfg.bib}
...
\centering
\caption{Comunicador AsTeRISCS Grid desarrollado por ARASAAC. Extraído de \cite{asterics_grid_comunicador}. \label{fig:comunicador}}
\end{figure}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Esquema de perceptrón simple. \label{fig:perceptronSimple}	}
\end{figure}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Esquema de perceptrón multicapa. \label{fig:perceptronMulticapa}	}
\end{figure}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{Esquema de la arquitectura de los transformers. \label{fig:transformers}}
\end{figure}
...
Para la realización de este trabajo se utilizarán modelos de lenguaje grandes preentrenados. Como se mencionó en el capítulo anterior, los modelos de lenguaje son modelos basados en redes neuronales profundas capaces de procesar y generar texto. En particular, los modelos de lenguaje grandes preentrenados han sido entrenados con grandes volúmenes de datos generales, pero aún no han sido ajustados para realizar tareas específicas. Este tipo de modelos han supuesto una revolución en el campo del procesamiento de lenguaje natural, ya que pueden ser adaptados para realizar una gran variedad de tareas específicas, incluso si estas difieren de los datos originales de entrenamiento. Utilizar LLM preentrenados supone un ahorro de tiempo y recursos significativo, evitando la necesidad de entrenar un modelo desde cero con un gran conjunto de datos.
...
En la Figura \ref{fig:matricesLoRA} se presenta un esquema de las matrices de parámetros de LoRA.
...
\node at (4.8, 2) {+};
\end{tikzpicture}
\caption{Esquema de las matrices de parámetros de LoRA. \label{fig:matricesLoRA}}
\end{figure}
...
\begin{table}
\caption{Interpretación de las puntuaciones de BLEU, extraída de \cite{googleTranslate}. \label{tab: bleu}}
\begin{center}
\begin{tabular}{  c | c  }
...
\begin{table}
\caption{Ejemplos de frases del conjunto de datos en español. \label{tab: ejemplosEspañol}}
\begin{center}
\begin{tabular}{ | l | }
\hline
	\\
...
\begin{table}
\caption{Ejemplos de frases del conjunto de datos en inglés. \label{tab: ejemplosInglés}}
\begin{center}
\begin{tabular}{ | l | }
\hline
	\\
...
\begin{table}
\caption{Formato \textit{prompt} de los datos. \label{tab: formatoPrompt}}
\begin{center}
\begin{tabular}{ | l | }
\hline
	\\
...
\begin{table}
\caption{Ejemplo de frase utilizando el formato de \textit{prompt} definido. \label{tab: ejemploPrompt}}
\begin{center}
\begin{tabular}{ | l | }
\hline
	\\
...
\begin{table}
\caption{Ejemplo de frase utilizando el formato de \textit{prompt} definido para la inferencia. \label{tab: formatoInferencia}}
\begin{center}
\begin{tabular}{ | l | }
\hline
	\\
...
Para evaluar los modelos, se utilizará la técnica de validación cruzada, que consiste en realizar diferentes particiones de los datos en conjuntos de entrenamiento y validación, entrenando y evaluando de manera independiente cada partición. Por esta razón, los datos se han dividido en un conjunto de entrenamiento y validación para validación cruzada y un conjunto adicional de test. Este último no se utilizará durante la fase de entrenamiento, sino exclusivamente para la comparación final de resultados entre los modelos. El conjunto de datos de test ha sido seleccionado de manera aleatoria, considerando solo las frases que no presentaban valores nulos.

La Tabla \ref{tab:frases} muestra la cantidad de datos utilizados para los experimentos en español y en inglés, tanto para el entrenamiento y validación de los modelos como para su posterior evaluación en el conjunto de test.

\begin{table}
\caption{Número de frases para entrenamiento y validación de los conjuntos de datos en español y en inglés. \label{tab:frases}}
\begin{center}
\begin{tabular}{  c || c  c  }
	\ & Español & Inglés \\
	\hline
	Entrenamiento y validación & 290 & 220 \\
	Test & 50 & 40
\end{tabular}
\end{center}
\end{table}
...
\begin{table}
\caption{BLEU y COMET para los modelos Gemma-7B y Llama3-8B en español. \label{tab: Gemma y Llama}}
\begin{center}
\begin{tabular}{ c c c c c c }
	\ & \ & \multicolumn{2}{c}{Gemma-7B} & \multicolumn{2}{c}{Llama3-8B}\\
	\hline
...
\mycomment{
\begin{table}
\caption{Puntuación de BLEU y COMET para el modelo Llama3-8B. \label{tab: Llama exploración de alpha}}
\begin{center}
...
\caption{Resultados de BLEU y COMET obtenidos con el modelo Llama3-8B tras ser entrenado mediante adaptación con LoRA con los valores de $r$ 16, 32, 64, 128 y 256;  y explorando distintas configuraciones de \textit{alpha} con el conjunto de datos en español. \label{fig:graficasLlama}}
\end{figure}
...
\subsection{Evaluación del modelo final en el conjunto de test}
...
Aunque los valores obtenidos para BLEU y COMET son inferiores a los resultados alcanzados durante la fase de experimentación anterior, estos resultados se encuentran dentro de un rango adecuado de las métricas. En la mayoría de casos, el modelo genera frases que se ajustan bien a las palabras clave proporcionadas, aunque a veces presenta errores relacionados con los tiempos verbales u otras pequeñas confusiones. La disminución en las métricas puede deberse a la diferencia entre el conjunto de entrenamiento y el conjunto de test. A pesar de esto, los resultados siguen siendo prometedores y reflejan un buen desempeño del modelo en esta tarea.

\subsection{Comparación con los resultados obtenidos mediante \textit{prompting}}

Se ha realizado un experimento adicional para evaluar el modelo Llama3-8B sin entrenamiento previo, con el objetivo de medir la mejora de este tras ser entrenado con el conjunto de datos de entrenamiento. Para ello, se introdujeron al modelo algunos datos específicos obtenidos de forma aleatoria del conjunto de entrenamiento mediante \textit{prompting}, empleando las técnicas descritas en la Sección \ref{prompting} de \textit{zero-shot prompting} y \textit{few-shot prompting}. La evaluación de estos experimentos se ha realizado utilizando el conjunto reservado para test.

La Tabla \ref{tab:promptingEspañol} resume los resultados obtenidos con las técnicas de \textit{zero-shot prompting} y \textit{few-shot prompting}, así como los resultados del modelo final de Llama3 entrenado mediante LoRA. Además, en la Tabla \ref{tab:ejemplosPromptingEspañol} se presentan dos ejemplos de generaciones realizadas mediante estos tres métodos.

\begin{table}
\caption{BLEU y COMET del modelo Llama3 utilizado mediante \textit{zero-shot prompting}, \textit{few-shot prompting} y entrenamiento mediante LoRA con el conjunto de datos en español. \label{tab:promptingEspañol}}
\begin{center}
\begin{tabular}{ c | c c }
	\ & BLEU & COMET \\
	\hline
...
\begin{table}
\caption{Ejemplos de frases generadas mediante \textit{zero-shot prompting}, \textit{few-shot prompting} y entrenamiento mediante LoRA con el conjunto de datos en español. \label{tab:ejemplosPromptingEspañol}}
\begin{center}
\begin{tabular}{ c | c | c }
	\ & Lemas & Frase \\
	\hline
	\hline
	\textit{Zero-shot prompting} &  & Juan bajar planta \\
	\textit{Few-shot prompting} & Juan, bajar, planta & Juan baja a la planta baja.  \\
	LoRA &  & Juan bajó la planta.  \\
	\hline
	\textit{Zero-shot prompting} &  & Voy a salir de aquí, a la cabaña. \\
	\textit{Few-shot prompting} & seis, cabritillos, salir, contento & Los seis cabritillos salen contentos.  \\
	LoRA &  & Los seis cabritillos salen contentos.  \\

\end{tabular}
\end{center}
\end{table}

En el caso de \textit{zero-shot prompting}, los resultados son considerablemente bajos, lo que evidencia que el modelo no comprende adecuadamente la tarea en la mayoría de los casos. Se identificaron tres tipos de respuesta:

\begin{enumerate}
	\item \textbf{Repetición de las palabras clave}: en la mayoría de los casos, el modelo se limita a repetir las palabras clave que fueron introducidas, ya sea de manera exactamente igual o con pequeñas modificaciones como la eliminación de comas, la adición de comillas o el uso de mayúscula en ciertas palabras. Este comportamiento se observa en el primer ejemplo de la Tabla \ref{tab:ejemplosPromptingEspañol}.
	\item \textbf{Frases sin sentido o sin relación con las palabras clave}: en otros casos, el modelo genera frases utilizando las palabras clave pero que no tienen sentido o no guardan relación con las palabras introducidas. Es el caso del segundo ejemplo de la Tabla \ref{tab:ejemplosPromptingEspañol}.
	\item \textbf{Frases similares a las esperadas}: en algunas ocasiones, el modelo logra generar frases que coinciden con las reales o que son similares a estas.
\end{enumerate}

Para la técnica de \textit{few-shot prompting}, se proporcionaron al modelo cinco ejemplos de palabras clave con sus correspondientes frases generadas. En este caso, el modelo demostró una mejor comprensión de la tarea, generando resultados que, en la mayoría de casos, se acercan más a los reales en comparación con el  \textit{zero-shot prompting}. Sin embargo, presenta mayor cantidad de errores que el modelo entrenado con datos específicos. Algunos de estos errores incluyen cambios en el orden de las palabras o la adición de palabras que no aparecen en la lista de palabras clave, como se puede ver en el primer ejemplo de la Tabla \ref{tab:ejemplosPromptingEspañol}.
...
Para evaluar la influencia del idioma en los resultados de los modelos, se han replicado los mismos experimentos realizados con el conjunto de datos en español utilizando el conjunto en inglés. De nuevo, se han utilizado los modelos Gemma-7B y Llama3-8B disponibles en el portal de \textit{Hugging Face}. Al igual que en los experimentos anteriores, se ha aplicado la adaptación LoRA y se ha llevado a cabo el entrenamiento utilizando validación cruzada con un total de 10 particiones.

En primer lugar, se han realizado los experimentos para ambos modelos explorando diferentes valores de $r$ (16, 32 y 64) y utilizando una configuración fija de $\alpha = 2r$. La Tabla \ref{tab: Gemma y Llama inglés} muestra los resultados obtenidos de BLEU y COMET de los modelos Gemma-7B y Llama3-8B entrenados con el conjunto de datos en inglés para estos valores de rango y \textit{alpha}.

\begin{table}
\caption{BLEU y COMET para los modelos Gemma-7B y Llama3-8B en inglés. \label{tab: Gemma y Llama inglés}}
\begin{center}
\begin{tabular}{ c c c c c c }
	\ & \ & \multicolumn{2}{c}{Gemma-7B} & \multicolumn{2}{c}{Llama3-8B}\\
	\hline
	...
	\hline
	\hline
	16 & 32 & 44.36 & 85.94 & 61.84 & 90.86 \\
	\hline
	32 & 64 & 54.23 & 88.88 & 62.22 & 91.11 \\
	\hline
	64 & 128 & 47.51 & 84.10 & 63.42 & 90.85 \\
...
Al igual que ocurría con el conjunto en español, los resultados obtenidos con el modelo Gemma son notablemente inferiores a los resultados obtenidos con el modelo Llama3.

\begin{table}
\caption{Mejores resultados de BLEU y COMET en los experimentos de ambos idiomas y modelos. \label{tab:comparacionIdiomas}}
\begin{center}
\begin{tabular}{ c c c c c c }
	 \ & \multicolumn{2}{c}{Gemma-7B} & \multicolumn{2}{c}{Llama3-8B}\\
	\hline
	 \textit{Idioma} & BLEU & COMET & BLEU & COMET \\
	\hline
	\hline
	 Español & 52.44   &    88.92 &  66.13   &   93.37 \\
	\hline
	 Inglés & 54.23    &    88.88 & 62.22    &   91.12 \\
...
En la Tabla \ref{tab:comparacionIdiomas} se muestran los mejores resultados obtenidos de BLEU y COMET para cada modelo e idioma. Se han tenido en cuenta los experimentos realizados con los valores de rango 16, 32 y 64 y la configuración de $\alpha = 2r$. En el caso de los experimentos en español, el mejor valor de rango entre estos es $r = 32$ para ambos modelos. Para el conjunto en inglés, también es $r = 32$ para Llama3 y para Gemma.
...
\mycomment{
\begin{table}
\caption{Puntuación de BLEU y COMET para el modelo Llama3-8B con el conjunto de datos en inglés. \label{tab: Llama exploración de alpha inglés}}
\begin{center}
\begin{tabular}{ c c | c c }
	\hline
	\textit{Rank} & \textit{Alpha} & BLEU & COMET \\
...
\caption{Resultados de BLEU y COMET obtenidos con el modelo Llama3-8B tras ser entrenados mediante adaptación con LoRA con los valores de $r$ = 16, 32, 64, 128 y 256;  y explorando distintas configuraciones de \textit{alpha} con el conjunto de datos en inglés. \label{fig:graficasLlama inglés}}
\end{figure}
...
Al igual que con el conjunto en español, el experimento final con los datos en inglés ha consistido en el entrenamiento de un modelo Llama3 utilizando la mejor configuración de parámetros identificada durante la experimentación previa, que fue $r = 128$ y $\alpha = 2r$.
...
La Tabla \ref{tab:promptingInglés} muestra los resultados de BLEU y COMET obtenidos con ambas técnicas, así como los resultados del modelo final Llama3 entrenado mediante LoRA con el conjunto de datos de entrenamiento en inglés. Además, en la Tabla \ref{tab:ejemplosPromptingInglés} se proporcionan dos ejemplos de frases generadas utilizando cada una de estas técnicas.

\begin{table}
\caption{BLEU y COMET del modelo Llama3 utilizado mediante \textit{zero-shot prompting}, \textit{few-shot prompting} y entrenamiento mediante LoRA con el conjunto de datos en inglés. \label{tab:promptingInglés}}
\begin{center}
\begin{tabular}{ c | c c }
	\ & BLEU & COMET \\
	\hline
...
\begin{table}
\caption{Ejemplos de frases generadas mediante \textit{zero-shot prompting}, \textit{few-shot prompting} y entrenamiento mediante LoRA con el conjunto de datos en español. \label{tab:ejemplosPromptingInglés}}
\begin{center}
\begin{tabular}{ c | c | c }
	\ & Lemas & Frase \\
	\hline
	\hline
	\textit{Zero-shot prompting} &  & child play boat \\
	\textit{Few-shot prompting} & child, play, boat & The child plays with the boat.  \\
	LoRA &  & The child plays with the boat. \\
	\hline
	\textit{Zero-shot prompting} &  & In the dining room, there are a fork \\
	\textit{Few-shot prompting} & fork, be, dining, room & I am in the dining room. \\
	LoRA &  & The fork is in the dining room. \\
...
Al igual que en el caso del modelo en español, los resultados obtenidos con \textit{zero-shot prompting} son significativamente inferiores a los de las otras técnicas, indicando que el modelo tiene grandes problemas para comprender la tarea adecuadamente. En la mayoría de los casos, el modelo se limita a repetir los lemas introducidos, a veces con ligeras variaciones. Un ejemplo de esto se puede observar en el primer caso de la Tabla \ref{tab:ejemplosPromptingInglés}. En otros casos, el modelo genera frases que no guardan relación con las palabras proporcionadas, cambian el orden de estas o presentan errores. Por ejemplo, en el segundo caso de la Tabla \ref{tab:ejemplosPromptingInglés}, el modelo generó una frase con las palabras en un orden diferente al que aparecían en la lista de lemas y con un error de conjugación del verbo. En muy pocas ocasiones, la frase generada por el modelo se asemeja a la frase real.

En el caso del \textit{few-shot prompting}, se proporcionaron al modelo cinco ejemplos de listas de palabras clave con sus correspondientes frases. El modelo muestra un rendimiento mucho mejor, demostrando que es capaz de comprender el objetivo de la tarea. Sin embargo, en algunas ocasiones el modelo añade palabras que no aparecían en la lista de palabras clave o se deja palabras de la lista para crear la frase. Por ejemplo, en el segundo caso de la Tabla \ref{tab:ejemplosPromptingInglés} se observa que para \textit{few-shot prompting}, el modelo no incluye la palabra \textit{fork} en la frase, a pesar de que esta aparece en la lista de lemas proporcionada. Aún así, el modelo generó frases cercanas a las frases reales en la mayoría de ocasiones. Los errores cometidos son similares a los del modelo entrenado con LoRA, y la puntuación de COMET obtenida con \textit{few-shot prompting} es ligeramente superior a la del modelo entrenado con datos específicos.

Así, los resultados de la Tabla \ref{tab:promptingInglés} indican que, aunque el desempeño de \textit{zero-shot prompting} es muy bajo, \textit{few-shot prompting} resulta muy eficaz. Los resultados obtenidos con esta técnica son muy similares a los obtenidos con el modelo entrenado mediante LoRA en términos de BLEU, y superiores en cuanto a COMET. Esto podría sugerir una posible limitación en la calidad del conjunto de datos en inglés utilizado para el entrenamiento. Además, se observa que Llama3 está mejor entrenado para generar frases en inglés incluso con \textit{few-shot prompting}, ya que la mayoría de los datos utilizados para su entrenamiento son en inglés.
...
\section{Comparación con personas}
...
\centering
\caption{Resultados de BLEU y COMET obtenidos por los individuos y por el modelo en la tarea de generación de frases. \label{fig:comparacionIndividuos}}
\end{figure}
...
\begin{table}
\caption{Frases generadas por Llama3 con errores en el tiempo verbal. \label{tab:erroresTiempoVerbal}}
\begin{center}
\begin{tabular}{ c | c | c }
...
\begin{table}
\caption{Frases generadas por Llama3 con errores en preposiciones. \label{tab:erroresPreposicion}}
\begin{center}
\begin{tabular}{ c | c | c }
	\ Lemmas & Frase generada & Frase real \\
	\hline
...
\begin{table}
\caption{Frases generadas por Llama3 con errores en el sujeto. \label{tab:erroresSujeto}}
\begin{center}
\begin{tabular}{ c | c | c }
	\ Lemmas & Frase generada & Frase real \\
	\hline
...
\begin{table}
\caption{Frases generadas por Llama3 con varios errores. \label{tab:erroresVariados}}
\begin{center}
\begin{tabular}{ c | c | c }
	\ Lemmas & Frase generada & Frase real \\
	\hline
...
\begin{enumerate}
	...
	\item Se han adaptado dos modelos de lenguaje grandes de actualidad, Gemma y Llama3, para la tarea de generación de frases sintácticamente correctas a partir de palabras clave para su uso en comunicadores electrónicos. Ambos modelos han sido evaluados en dos idiomas: español e inglés. Tras la realización de los experimentos, se determinó que el modelo Llama3 ofrece mejores resultados que Gemma. Los resultados obtenidos resultan prometedores.
...
El entrenamiento de  modelos de lenguaje grandes, que forma parte del campo del Procesamiento de Lenguaje Natural, se ha basado en conceptos estudiados en la carrera, por ejemplo el \textit{fine-tuning} y la técnica de validación cruzada.
